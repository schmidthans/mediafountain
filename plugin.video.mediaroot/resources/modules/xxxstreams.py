...
def CATEGORIES():
    ...
    main.addDir('Full Movies',base_url +'/full-porn-movie-stream/page/0','xxxstreamsIndex',artwork + '/main/recentvideos.png')
...
def VIDEOLINKS(name,url,thumb):
    ...
    if match:
        match1=re.compile('href="(http[s]?://[^www.pix|^www.theteen|^sharenxs]?.*?)["<\s].*?target="_blank">([^<]+?)<', re.S).findall(match[0].replace('/tttr/1out2.php?s=0,100,100,100:*&amp;u=',''))
